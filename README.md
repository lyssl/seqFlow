...
以下为本地环境下的初步压测结果，仅供参考。实际性能会受到 CPU、JDK、数据库、网络、连接池和 `step` 配置影响。
...
| 号段步长 | 数据库号段申请次数 | 总耗时 | TPS | 成功数 | 唯一 ID 数 | 重复 ID 数 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
从结果可以看到，`step` 越大，数据库号段申请次数越少，整体吞吐量越高。生产环境应根据并发量、可接受的号段浪费和数据库压力选择合适的 `step`。
